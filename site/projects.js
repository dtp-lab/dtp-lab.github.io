(async function () {
  const { loadJson, showDataError, applyPageHeading } = DTPLab;
  ...
  const render = (category = "all") => {
    ...
    const current = filtered.filter((project) => project.status === "current");
    const completed = filtered.filter((project) => project.status === "completed");
