(async function () {
  ...
  try {
    ...
    const metaIcon = (icon) => `<span class="seminar-meta-icon">${icon}</span>`;
    root.innerHTML = Object.entries(groups).map(([year, seminars]) => `<section class="year-group"><header class="year-heading"><h2>${escapeHtml(year.trim())}</h2><span>${seminars.length}회</span></header><div class="seminar-list">${seminars.map((seminar) => `<article class="seminar-card"><div class="seminar-title-row"><span class="seminar-title-icon">${presentationIcon}</span><h3 class="seminar-title">${escapeHtml(seminar.title)}</h3></div><div class="seminar-meta-row"><span class="seminar-meta-item seminar-date">${metaIcon(calendarIcon)}<time datetime="${escapeHtml(seminar.date)}">${escapeHtml(seminar.date)}</time></span><span class="seminar-meta-item seminar-speaker">${metaIcon(speakerIcon)}<span class="speaker">${escapeHtml(seminar.speaker)}</span></span>${seminar.keywords?.length ? `<span class="seminar-meta-item seminar-keywords">${metaIcon(keywordIcon)}${renderKeywords(seminar.keywords)}</span>` : ""}</div><p class="seminar-summary">${escapeHtml(seminar.summary)}</p></article>`).join("")}</div></section>`).join("");
